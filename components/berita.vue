...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      const ENDPOINT = 'http://localhost:8055/items/News';
      try {
        ...
        <div class="flex flex-col md:flex-row gap-8 mb-8 md:mb-16">
          <div class="w-full sm:w-1/3 flex justify-center">
            <img
              ...
              :src="`http://localhost:8055/assets/${slides[0].image}`"
              alt="" />
          </div>
          <div class="md:w-1/2">
            <div class="flex flex-row items-center">
              <Icon name="mdi:calendar" color="#063E63" />
              <p class="text-[#063E63] text-sm sm:text-base">
                {{ `${slides[0].publication_date}` }}
              </p>
            </div>
            <h1
              v-html="slides[0].title"
              class="text-base md:text-lg lg:text-2xl text-black font-semibold my-2 md:my-6"></h1>
            <p
              v-html="slides[0].content"
              class="text-gray-500 text-xs sm:text-sm md:text-base line-clamp-4"></p>
          </div>
        </div>
        <carousel
          v-if="isMediumSize"
          ...
          <slide v-for="slide in slides" :key="slide.id">
            <div class="flex flex-col w-full h-full">
              <img
                ...
                alt="`Slide ${{slide.id}}`"
                :src="`http://localhost:8055/assets/${slide.image}`" />
              <p
              ...
              <p
                ...
                class="text-black text-xs md:text-sm font-light line-clamp-4 text-justify mt-2"></p>
            </div>
          </slide>
          <template #addons>
            <navigation />
          ...
          <div
            ...
            <div class="w-1/3 mr-2 flex">
              <img
                ...
                :src="`http://localhost:8055/assets/${slides[i].image}`"
                alt="" />
            </div>
            <div class="w-2/3">
              ...
              <p
                v-html="slides[i].content"
                class="text-xs text-black font-light line-clamp-3"></p>
            </div>
          </div>
        </div>
      </div>
